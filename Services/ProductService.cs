...
public class ProductService
{
    ...
    public async Task<Product?> GetByIdAsync(int id) => await _context.Products.Include(p => p.Category).FirstOrDefaultAsync(m => m.Id == id);
    public async Task<List<Product>> GetPremiumAsync(int count) => await _context.Products.Where(p => p.IsAvailable == true).OrderByDescending(p => p.Price).Take(count).ToListAsync();
    public async Task<List<Product>> GetByCategoryAsync(int? categoryId) => await _context.Products.Where(p => p.CategoryId == categoryId).ToListAsync();
