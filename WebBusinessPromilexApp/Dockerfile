...
WORKDIR /src
COPY ["WebBusinessPromilexApp/WebBusinessPromilexApp.csproj", "WebBusinessPromilexApp/"]
RUN dotnet restore "./WebBusinessPromilexApp/WebBusinessPromilexApp.csproj"
COPY . .
WORKDIR "/src/WebBusinessPromilexApp"
RUN dotnet build "./WebBusinessPromilexApp.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./WebBusinessPromilexApp.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "WebBusinessPromilexApp.dll"]